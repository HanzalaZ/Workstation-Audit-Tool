...
if ($ComputerSystem.PartOfDomain) {
    ...
    try {
        ...
        $JoinDate = "Unknown"
        if ($DebugOutput) { Write-Host "    [Debug] ADSI lookup failed: $($_.Exception.Message)" -ForegroundColor Gray }
        ...
        try {
            ...
            if ($DebugOutput) { Write-Host "    [Debug] Registry lookup also failed: $($_.Exception.Message)" -ForegroundColor Gray }
        }
    }
}
